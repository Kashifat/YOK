...
SERVICES: list[ServiceSpec] = [
    ...
    ServiceSpec("NOTIFICATION", "MICROSERVICES.NOTIFICATION.main:app", 8004),
    ...
    ServiceSpec("PAIEMENT_VENDEURS", "MICROSERVICES.PAIEMENT_VENDEURS.main:app", 8008),
    ServiceSpec("LIVRAISON", "MICROSERVICES.LIVRAISON.main:app", 8009),
    ServiceSpec("LOGISTIQUE", "MICROSERVICES.LOGISTIQUE.main:app", 8013),
    ServiceSpec("RESEAU_SOCIAUX", "MICROSERVICES.RESEAU_SOCIAUX.main:app", 8010),
    ServiceSpec("AVIS", "MICROSERVICES.AVIS.main:app", 8011),
    ServiceSpec("PAIEMENT_COMPAT", "MICROSERVICES.PAIEMENT.main:app", 8012),
]
...
def main():
    parser = argparse.ArgumentParser(description="Lancer tous les microservices YOK en parallèle")
    parser.add_argument(
        "--reload",
        action="store_true",
    ...
    ports_occupes = [svc for svc in SERVICES if is_port_in_use(svc.port)]
    if ports_occupes:
        print("Ports déjà occupés. Libère-les avant de lancer tous les services :")
        for svc in ports_occupes:
            print(f"- {svc.name}: {svc.port}")
        sys.exit(1)

    print("Démarrage des microservices YOK...")
    ...
    try:
        ...
        print("Appuie sur Ctrl+C ici pour arrêter tous les services.\n")

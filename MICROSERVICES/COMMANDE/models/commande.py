...
class Commande(Base):
	...
	code_promo_identifiant = Column(UUID(as_uuid=True), ForeignKey("codes_promo.identifiant"), nullable=True)
	...
	frais_livraison_cfa = Column(Integer, default=0, nullable=False)
	montant_remise_cfa = Column(Integer, default=0, nullable=False)
...
class CommandeArticle(Base):
	...
	taille_selectionnee = Column(Text)
	couleur_selectionnee = Column(Text)
